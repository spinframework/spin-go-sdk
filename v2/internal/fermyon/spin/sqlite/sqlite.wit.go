...
// Package sqlite represents the imported interface "fermyon:spin/sqlite".
package sqlite
...
// Error represents the variant "fermyon:spin/sqlite#error".
//
...
// Value represents the variant "fermyon:spin/sqlite#value".
//
//	variant value {
//		integer(s64),
//		real(f64),
...
// RowResult represents the record "fermyon:spin/sqlite#row-result".
//
...
// QueryResult represents the record "fermyon:spin/sqlite#query-result".
//
// A result of a query
//
//	record query-result {
...
//
//go:nosplit
func Open(database string) (result cm.Result[ErrorShape, Connection, Error]) {
	database0, database1 := cm.LowerString(database)
	wasmimport_Open((*uint8)(database0), (uint32)(database1), &result)
	return
}
...
//
//	execute: func(conn: connection, statement: string, parameters: list<value>) ->
//	result<query-result, error>
//
//go:nosplit
func Execute(conn Connection, statement string, parameters cm.List[Value]) (result cm.Result[QueryResultShape, QueryResult, Error]) {
	conn0 := (uint32)(conn)
	statement0, statement1 := cm.LowerString(statement)
	parameters0, parameters1 := cm.LowerList(parameters)
	wasmimport_Execute((uint32)(conn0), (*uint8)(statement0), (uint32)(statement1), (*Value)(parameters0), (uint32)(parameters1), &result)
	return
}
